...
# Create issue under an epic or parent issue
jira issue create -tStory -pPROJ --parent PROJ-123 -s"Story summary"
...
**Epic / parent handling:** Use `--parent EPIC-KEY` when creating a Story or Task that belongs under an epic. Do not use `--custom` to set Epic Link.
...
# Create a task under a parent epic/issue
jira issue create \
  --no-input \
  -tTask \
  -pPROJ \
